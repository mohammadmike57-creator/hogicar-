...
const SearchWidget: React.FC<SearchWidgetProps> = ({ initialValues, onSearch, showTitle = false }) => {
    ...
    // Removed default locations fetch as per user request to start empty
    ...
    const handleLocationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (value.length < 3) {
            setSuggestions([]); // Clear suggestions if less than 3 chars
    ...
    const handleDropoffLocationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (value.length < 3) {
            setDropoffSuggestions([]); // Clear suggestions if less than 3 chars
