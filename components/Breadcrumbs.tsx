  ...
  return (
    <>
      ...
      <nav aria-label="Breadcrumb" className="flex items-center space-x-2 text-[10px] sm:text-xs font-black uppercase tracking-widest mb-6 overflow-x-auto no-scrollbar whitespace-nowrap">
        <Link 
          to="/" 
          className={`flex items-center gap-1.5 transition-colors ${isLight ? 'text-white/80 hover:text-white' : 'text-slate-600 hover:text-slate-900'}`}
        >
        ...
        {items.map((item, index) => (
          <React.Fragment key={index}>
            <ChevronRight className={`w-3.5 h-3.5 ${isLight ? 'text-white/40' : 'text-slate-400'}`} aria-hidden="true" />
            <Link 
              to={item.route} 
