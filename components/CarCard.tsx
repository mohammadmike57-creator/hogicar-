...
const CarCard: React.FC<CarCardProps> = ({ car, cars, days, startDate, endDate, pickupCode, dropoffCode }) => {
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl shadow-sm hover:shadow-2xl border-2 border-[#008009] hover:border-[#00a30b] transition-all duration-500 w-full group/card overflow-hidden flex flex-col h-full hover:-translate-y-1 md:scale-[0.97] md:hover:scale-100">
          {/* Header Badge */}
          {car.hogicarChoice && (
            <div className="bg-gradient-to-r from-[#008009] via-[#00a30b] to-[#008009] text-white px-4 md:py-1.5 py-1 flex items-center justify-center gap-2">
                <Award className="w-3 md:w-3.5 h-3 md:h-3.5 text-white fill-white/20" />
                <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest">Hogicar Recommended</span>
            </div>
          )}

          <div className="flex flex-col md:flex-row flex-grow">
              {/* Car Image Area */}
              <div className="relative md:w-1/4 bg-white border-b md:border-b-0 md:border-r border-slate-50 flex flex-col p-2 md:p-3 group/img">
                  <Link to={`/car/${car.id}?${searchParams}`} state={{ cars: cars }} onClick={handleSelectCar} className="w-full aspect-[16/10] flex items-center justify-center mb-2 md:mb-4">
                      <img 
                        src={displayImage} 
                  ...
                  <div className="flex items-center justify-between gap-1 pt-2 md:pt-4 border-t border-slate-100 mt-auto w-full">
                      <img
                          src={car.supplier.logo || (car.supplier as any).logoUrl}
                          alt={car.supplier.name}
                          className="h-6 md:h-8 w-auto object-contain max-w-[60px] md:max-w-[90px]"
                      />
                      <div 
                        className="flex items-center gap-1 md:gap-2 group/rating relative cursor-pointer"
                      ...
                      >
                          <div className="flex flex-col items-end">
                            <span className="text-[9px] md:text-[11px] font-black text-slate-900 leading-none mb-0.5">
                          ...
                          <div className="bg-[#008009] text-white text-[11px] md:text-[14px] font-black w-7 h-7 md:w-9 md:h-9 flex items-center justify-center rounded md:rounded-lg shadow-sm shrink-0">
                              {car.supplier.rating}
                          </div>
              ...
              <div className="flex-grow flex flex-col md:flex-row">
                  <div className="p-3 md:p-3 flex-grow border-b md:border-b-0 md:border-r border-slate-50">
                      {/* Title & Category */}
                      <div className="mb-2 md:mb-4">
                          ...
                          <Link to={`/car/${car.id}?${searchParams}`} state={{ cars: cars }} onClick={handleSelectCar}>
                              <h3 className="text-base md:text-[1rem] font-black text-slate-900 leading-tight hover:text-[#008009] transition-colors uppercase tracking-tight line-clamp-1">
                                  {car.displayName}
                              </h3>
                          </Link>
                          <p className="text-[8px] md:text-[10px] text-slate-400 font-bold flex items-center gap-1 mt-0.5">
                              or similar <Info className="w-2 md:w-2.5 h-2 md:h-2.5" />
                          </p>
                      </div>

                      {/* Specs Grid (Compact) */}
                      <div className="grid grid-cols-2 gap-x-2 md:gap-x-3 gap-y-1.5 md:gap-y-2 mb-2 md:mb-4 py-2 md:py-2.5 border-y border-slate-50 bg-slate-50/30 rounded-lg px-2 md:px-2.5">
                          <div className="flex items-center gap-1.5 md:gap-2.5 text-slate-600">
                              <Users className="w-3.5 md:w-3.5 h-3.5 md:h-3.5 text-slate-400"/>
                              <span className="text-[10px] md:text-[11px] font-bold">{car.passengers} <span>Adults</span></span>
                          </div>
                          <div className="flex items-center gap-1.5 md:gap-2.5 text-slate-600">
                              <Briefcase className="w-3.5 md:w-3.5 h-3.5 md:h-3.5 text-slate-400"/>
                              <span className="text-[10px] md:text-[11px] font-bold">{car.bags} <span>Bags</span></span>
                          </div>
                          <div className="flex items-center gap-1.5 md:gap-2.5 text-slate-600">
                              <div className="text-slate-400 scale-[0.8] md:scale-90"><AutomaticIcon /></div>
                              <span className="text-[10px] md:text-[11px] font-bold">
                                  {car.transmission === 'AUTOMATIC' ? 'Auto' : 'Manual'}
                              </span>
                          </div>
                          <div className="flex items-center gap-1.5 md:gap-2.5 text-slate-600">
                              <Wind className="w-3.5 md:w-3.5 h-3.5 md:h-3.5 text-slate-400"/>
                      ...
                      {recentBookingInfo.isRecent && (
                        <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-100 p-1.5 md:p-2 rounded-lg mt-2">
                           <Clock className="w-3 h-3 text-[#008009]" />
                           <span className="text-[9px] md:text-[10px] font-black text-[#008009] uppercase tracking-wider">{recentBookingInfo.message}</span>
                        </div>
                      )}
                  </div>

                  {/* Price & CTA Section */}
                  <div className="p-3 md:p-3 md:w-1/3 bg-slate-50/50 flex flex-col justify-between border-t md:border-t-0 md:border-l border-slate-100">
                      <div>
                          {/* Pricing Info */}
                          <div className="flex flex-col mb-2 md:mb-4">
                              <div className="flex items-center justify-between mb-1">
                                <p className="text-[8px] md:text-[9px] text-slate-400 font-black uppercase tracking-widest">Total <span>for {days} days</span></p>
                                {car.supplier.rating >= 4.5 && (
                                    <div className="flex items-center gap-1 text-[8px] md:text-[9px] font-black text-[#008009] uppercase bg-[#008009]/5 px-1 md:px-1.5 py-0.5 rounded">
                              ...
                              <div className="flex items-center gap-2">
                                  {car.promotionPercent > 0 && (
                                      <span className="text-[9px] md:text-[10px] text-slate-300 line-through font-bold">
                              ...
                              <p className="text-[8px] md:text-[9px] text-slate-400 font-bold mt-0.5 flex items-center gap-1">
                                  <Shield className="w-2.5 h-2.5" /> All taxes included
                              </p>
                          </div>
                          
                          <div className="mb-2 md:mb-6 p-2 md:p-3 bg-[#008009]/5 rounded-xl border border-[#008009]/10">
                              <p className="text-[8px] md:text-[9px] text-emerald-700 font-black uppercase tracking-widest mb-1 flex items-center gap-1">
                                  <CreditCardIcon className="w-2.5 md:w-3 h-2.5 md:h-3" /> Pay Now
                              </p>
                              <div className="flex items-baseline gap-1">
                                  ...
                                  <span className="text-[8px] md:text-[10px] text-[#008009]/60 font-bold italic">to secure car</span>
                              </div>
                          </div>
                      </div>
                      ...
                      <div className="space-y-2 md:space-y-4">
                          ...
                          <Link 
                            ...
                            className="group/btn block w-full bg-[#008009] hover:bg-[#006607] text-white font-black py-3 md:py-3 rounded-lg md:rounded-xl shadow-md md:shadow-[0_6px_15px_-4px_rgba(0,128,9,0.3)] hover:shadow-lg transition-all active:scale-[0.98] text-center text-[11px] md:text-[10px] uppercase tracking-widest relative overflow-hidden"
                          >
                          ...
                          {/* Badges Footer */}
                          <div className="flex items-center justify-between gap-2 pt-1 md:pt-2 border-t border-slate-200/60 flex">
