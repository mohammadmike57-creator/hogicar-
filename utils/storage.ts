...
const stripLargeDataUrls = (value: any, seen = new WeakSet<object>()): any => {
  ...
  if (Array.isArray(value)) {
    return value.map(item => stripLargeDataUrls(item, seen));
  }

  return Object.entries(value).reduce((acc, [key, item]) => {
    if (typeof item !== 'function') {
      acc[key] = stripLargeDataUrls(item, seen);
    }
    return acc;
  }, {} as Record<string, any>);
};

export const compactCarForStorage = (car: Car): Car => stripLargeDataUrls(car) as Car;
...
export const persistSelectedCar = (car: Car, cars?: Car[]) => {
  const compactCar = compactCarForStorage(car);
  safeSessionStorageSetItem(SELECTED_CAR_ID_KEY, String(car.id));
  safeSessionStorageSetItem(SELECTED_CAR_KEY, JSON.stringify(compactCar));
