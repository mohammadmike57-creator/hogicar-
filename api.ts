...
const toLocationSuggestions = (locations: any[]): LocationSuggestion[] => {
  const results = locations.map((loc: any): LocationSuggestion => {
    const backendType = loc.type || loc.airportType;
    const isAirport = (backendType && String(backendType).toLowerCase().includes('airport')) ||
                      String(loc.name || '').toLowerCase().includes('airport');

    let label = loc.name || loc.iataCode || 'Location';
    if (loc.municipality) {
    ...
    return {
      ...
      countryCode: loc.isoCountry || loc.countryCode || '',
      type: isAirport ? 'airport' : 'city',
    };
  });

  return Array.from(new Map(results.map((item) => [item.label, item])).values());
};
...
export const fetchLocations = async (query: string): Promise<LocationSuggestion[]> => {
  ...
  try {
    ...
    return toLocationSuggestions(Array.isArray(response.data) ? response.data : []);
  } catch (error) {
...
export const getBookingByRef = async (bookingRef: string | number): Promise<Booking> => {
  try {
    const response = await publicAxios.get(`${API_BASE_URL}/api/bookings/ref/${encodeURIComponent(String(bookingRef))}`);
    return response.data;
  } catch (error) {
    if (/^\d+$/.test(String(bookingRef))) {
      return getBooking(Number(bookingRef));
    }
    throw error;
  }
};
...
export const requestModification = async (id: number, data: any): Promise<any> => {
  const response = await publicAxios.post(`${API_BASE_URL}/api/bookings/${id}/modify/request`, data);
  return response.data;
};

export const confirmModification = async (id: number): Promise<Booking> => {
  const response = await publicAxios.post(`${API_BASE_URL}/api/bookings/${id}/modify/confirm`);
  return response.data;
};

export const submitReview = async (id: number | string, reviewData: any): Promise<any> => {
...
export const supplierConfirm = async (id: number | string, supplierConfirmationNumber: string): Promise<Booking> => {
  const response = await publicAxios.post(`${API_BASE_URL}/api/bookings/${id}/supplier-confirm`, { supplierConfirmationNumber });
  return response.data;
};
