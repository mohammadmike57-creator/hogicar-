  ...
  return (
    ...
    <div className="bg-slate-50 min-h-screen py-6 sm:py-8 font-sans overflow-x-hidden text-slate-800 selection:bg-emerald-100">
      ...
      <div className="max-w-[1500px] mx-auto px-3 sm:px-6 lg:px-10">
        ...
        <div className="mb-6 overflow-hidden rounded-card border border-slate-200 bg-white shadow-[0_22px_60px_-42px_rgba(15,23,42,0.75)]">
          <div className="h-1.5 bg-slate-100">
            <div className={`h-full rounded-r-full bg-[#007ac2] transition-all duration-700 ${routeStep === 'details' ? 'w-1/2' : 'w-full'}`}></div>
          </div>
          <div className="p-4 sm:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-[11px] font-extrabold uppercase tracking-[0.22em] text-[#007ac2]">Checkout</p>
              <h1 className="mt-1 text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-950">{pageTitle}</h1>
              <p className="mt-2 max-w-2xl text-sm sm:text-base font-medium leading-relaxed text-slate-600">{pageDescription}</p>
            </div>
            <div className="grid grid-cols-2 gap-2 rounded-card bg-slate-100 p-1.5 sm:min-w-[360px]">
              <div className={`rounded-card px-3 py-3 text-center ${routeStep === 'details' ? 'bg-slate-950 text-white shadow-sm' : 'text-slate-500'}`}>
                <p className="text-[10px] font-extrabold uppercase tracking-[0.18em]">Step 1</p>
                <p className="mt-1 text-xs sm:text-sm font-extrabold">Driver details</p>
              </div>
              <div className={`rounded-card px-3 py-3 text-center ${routeStep === 'payment' ? 'bg-[#007ac2] text-white shadow-sm' : 'text-slate-500'}`}>
                <p className="text-[10px] font-extrabold uppercase tracking-[0.18em]">Step 2</p>
                <p className="mt-1 text-xs sm:text-sm font-extrabold">Payment</p>
              </div>
            </div>
          </div>
          <div className="mt-5 grid grid-cols-1 gap-3 border-t border-slate-100 pt-4 sm:grid-cols-3">
            <div className="flex items-center gap-3 rounded-card bg-slate-50 px-3 py-3">
              <ShieldCheck className="h-4 w-4 text-[#007ac2]" />
              <span className="text-xs font-extrabold uppercase tracking-[0.14em] text-slate-700">Secure checkout</span>
            </div>
            <div className="flex items-center gap-3 rounded-card bg-slate-50 px-3 py-3">
              <BadgeCheck className="h-4 w-4 text-[#007ac2]" />
              <span className="text-xs font-extrabold uppercase tracking-[0.14em] text-slate-700">Verified supplier</span>
            </div>
            <div className="flex items-center gap-3 rounded-card bg-slate-50 px-3 py-3">
              <Headphones className="h-4 w-4 text-indigo-600" />
              <span className="text-xs font-extrabold uppercase tracking-[0.14em] text-slate-700">Booking support</span>
            </div>
          </div>
          </div>
        </div>
        ...
        <form onSubmit={handleConfirmBooking} className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 xl:gap-10">
          ...
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            ...
            <div className="bg-white rounded-card shadow-[0_18px_45px_-32px_rgba(15,23,42,0.55)] border border-slate-200 p-4 sm:p-5 flex flex-col md:flex-row items-center gap-5 sm:gap-6 relative overflow-visible group">
               <div className="bg-gradient-to-b from-slate-50 to-white p-4 sm:p-6 rounded-card border border-slate-200 flex-shrink-0 relative overflow-hidden">
                   <img 
                    src={displayImage} 
                    alt={car.model} 
               ...
               <div className="flex-grow text-center md:text-left relative z-10">
                  <div className="flex items-center justify-center md:justify-start gap-2 sm:gap-3 mb-3 sm:mb-4 flex-wrap">
                      <span className="bg-slate-950 text-white text-[10px] font-extrabold px-3 py-1.5 rounded-full uppercase tracking-[0.2em] shadow-lg border border-white/10">
                      ...
                      <span className="bg-emerald-50 text-emerald-700 text-xs font-extrabold px-3 py-1.5 rounded-full uppercase tracking-[0.2em] border border-emerald-100">Ready for Pick-up</span>
                  </div>
                  <h1 className="text-[1.35rem] sm:text-3xl font-extrabold text-slate-950 leading-tight tracking-tight mb-3">{car.displayName || `${car.make} ${car.model}`}</h1>
                  <p className="text-[11px] sm:text-xs font-extrabold text-slate-500 uppercase tracking-[0.14em] sm:tracking-[0.22em] flex items-center justify-center md:justify-start gap-2 sm:gap-3 flex-wrap">
                  ...
                  {!car.isHogicarChoiceBranded || car.supplier.name !== 'Hogi Car Choice' ? (
                    <div className="flex items-center justify-center md:justify-start gap-4 sm:gap-6 mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-slate-200/70">
                        <div className="bg-white border border-slate-200 p-2.5 sm:p-3 rounded-card shadow-sm">
                          {supplierLogo === 'HOGICAR_CHOICE_LOGO' || car.supplier.name === 'Hogi Car Choice' ? (
                            ...
                            <div className="h-12 sm:h-16 w-24 bg-slate-50 flex items-center justify-center rounded-card border border-dashed border-slate-200">
                        ...
                        <div className="flex items-center gap-4 sm:gap-5">
                          <div>
                            <p className="text-[11px] sm:text-xs font-extrabold text-slate-500 uppercase tracking-[0.18em] sm:tracking-[0.2em] leading-none mb-2">Service Provider</p>
                            <p className="text-sm sm:text-base font-extrabold text-slate-800 uppercase tracking-[0.12em] sm:tracking-[0.15em]">{car.supplier.name}</p>
                          </div>
                          <div
                            className="flex items-center gap-3 bg-white px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-card shadow-sm border border-slate-200 ml-1 group/rating relative cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all active:scale-95"
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                          ...
                          >
                             <div className={`relative ${getRatingColor(car.supplier.rating)} text-white w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-card shadow-lg shadow-slate-200 overflow-hidden shrink-0 ring-2 ring-white transition-transform group-hover/rating:scale-110`}>
                                 <div className="absolute inset-0 bg-gradient-to-br from-white/40 to-transparent opacity-50" />
                                 <span className="relative z-10 text-[11px] sm:text-base font-extrabold tracking-tight">{car.supplier.rating}</span>
                             </div>
                             <div className="hidden sm:flex flex-col">
                                 <div className="flex items-center gap-1.5 mb-0.5">
                                     <span className={`text-xs font-extrabold leading-none ${getRatingTextColor(car.supplier.rating)} tracking-tight`}>{getRatingDescription(car.supplier.rating)}</span>
                                     <Info className="w-3 h-3 text-slate-300 group-hover/rating:text-slate-500 transition-colors" />
                                 </div>
                                 <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-[0.12em] flex items-center gap-1">
                                   <BadgeCheck className="w-2.5 h-2.5 text-[#007ac2]" />
                                   Verified
                                 </span>
                             </div>
                    ...
                    <div className="flex items-center justify-center md:justify-start gap-4 sm:gap-6 mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-slate-200/70">
                       <div className="bg-[#003580] border-2 border-amber-400 p-3 sm:p-4 rounded-card shadow-xl flex items-center justify-center">
                          <Award className="w-8 h-8 text-amber-400 fill-amber-400/20" />
                       </div>
                       <div>
                         <p className="text-[11px] sm:text-xs font-extrabold text-amber-600 uppercase tracking-[0.25em] mb-1 italic">Hogicar Choice</p>
                         <p className="text-sm sm:text-lg font-extrabold text-slate-900 tracking-tight uppercase">Exclusive Verified Fleet</p>
                       </div>
                    </div>
                  )}
               </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
              <div className="bg-white rounded-card border border-slate-200 p-4 sm:p-5 shadow-sm">
                ...
                <p className="text-sm sm:text-base font-semibold text-slate-900 flex items-start gap-2"><MapPin className="w-4 h-4 text-[#007ac2] mt-0.5" /> <span>{pickupLabel}</span></p>
              ...
              <div className="bg-white rounded-card border border-slate-200 p-4 sm:p-5 shadow-sm">
                ...
                <p className="text-sm sm:text-base font-semibold text-slate-900 flex items-start gap-2"><MapPin className="w-4 h-4 text-[#007ac2] mt-0.5" /> <span>{dropoffLabel}</span></p>
            ...
            <div className="bg-white rounded-card shadow-[0_18px_45px_-34px_rgba(15,23,42,0.5)] border border-slate-200 p-5 sm:p-7">
               <div className="mb-6 flex flex-col gap-4 border-b border-slate-100 pb-5 sm:flex-row sm:items-start sm:justify-between">
                 <div>
                   <p className="text-[11px] font-extrabold uppercase tracking-[0.22em] text-[#007ac2]">Main driver profile</p>
                  <h2 className="mt-1 text-xl sm:text-2xl font-extrabold text-slate-950 flex items-center gap-3"><User className="w-5 h-5 text-[#007ac2]"/> Driver information</h2>
                ...
                <div className="hidden sm:block rounded-card border border-emerald-100 bg-emerald-50 px-4 py-3 shadow-sm">
                  <p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-emerald-800 flex items-center gap-2"><Check className="w-3 h-3"/> Required for booking</p>
                  <p className="mt-1 text-sm font-extrabold text-slate-950">Name, email & mobile</p>
                 </div>
               </div>
               <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_280px]">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    <div className="group"><label className="block text-[13px] font-bold text-slate-600 mb-2 ml-1 group-focus-within:text-[#007ac2] transition-colors uppercase tracking-wide">First name</label><FormInput icon={User} type="text" placeholder="e.g. JOHN" value={firstName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFirstName(e.target.value.toUpperCase())} required /></div>
                    <div className="group"><label className="block text-[13px] font-bold text-slate-600 mb-2 ml-1 group-focus-within:text-[#007ac2] transition-colors uppercase tracking-wide">Last name</label><FormInput icon={User} type="text" placeholder="e.g. DOE" value={lastName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setLastName(e.target.value.toUpperCase())} required /></div>
                    <div className="group"><label className="block text-[13px] font-bold text-slate-600 mb-2 ml-1 group-focus-within:text-[#007ac2] transition-colors uppercase tracking-wide">Email address</label><FormInput icon={Mail} type="email" placeholder="john.doe@example.com" value={email} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEmail(e.target.value.toUpperCase())} required /></div>
                    <div className="group"><label className="block text-[13px] font-bold text-slate-600 mb-2 ml-1 group-focus-within:text-[#007ac2] transition-colors uppercase tracking-wide">Mobile number</label><FormInput icon={Phone} type="tel" placeholder="+1..." value={phoneNumber} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPhoneNumber(e.target.value)} required /></div>
                    <div className="md:col-span-2 group pt-2">
                      <label className="block text-[13px] font-bold text-slate-600 mb-2 ml-1 group-focus-within:text-[#007ac2] transition-colors uppercase tracking-wide">Flight number <span className="text-[10px] text-slate-400 ml-2">(Highly Recommended)</span></label>
                      ...
                      <p className="text-sm text-slate-500 mt-4 font-medium flex items-start gap-3 bg-slate-50 p-4 rounded-card border border-slate-100"><Info className="w-5 h-5 text-[#007ac2] mt-0.5 flex-shrink-0"/> <span>Providing your flight number helps the rental provider monitor your arrival and hold your vehicle in case of delays.</span></p>
                    </div>
                 </div>
                 <div className="rounded-card border border-slate-200 bg-slate-50 p-5 shadow-sm">
                   <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-slate-500 mb-4 flex items-center gap-2"><ShieldCheck className="w-4 h-4" /> Checklist for pick-up</p>
                   <div className="space-y-4 text-sm font-bold text-slate-700">
                     <p className="flex items-center gap-3 bg-white p-3 rounded-card border border-slate-100 shadow-sm"><Check className="h-4 w-4 text-[#007ac2]" /> Valid driving license</p>
                     <p className="flex items-center gap-3 bg-white p-3 rounded-card border border-slate-100 shadow-sm"><Check className="h-4 w-4 text-[#007ac2]" /> Passport or ID</p>
                     <p className="flex items-center gap-3 bg-white p-3 rounded-card border border-slate-100 shadow-sm"><Check className="h-4 w-4 text-[#007ac2]" /> Driver's credit card</p>
                   </div>
                   <div className="mt-6 rounded-card border border-blue-100 bg-blue-50/50 p-4">
                     <p className="text-[11px] font-bold leading-relaxed text-[#007ac2]">These details will be used for your rental agreement and secure payment record.</p>
                   </div>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-white rounded-card shadow-[0_18px_45px_-34px_rgba(15,23,42,0.5)] border border-slate-200 p-5 sm:p-7">
               ...
               {createAccount && (
                  <div className="group">
                    <label className="block text-sm font-semibold text-slate-700 mb-2 ml-1 group-focus-within:text-[#007ac2] transition-colors">Create password <span className="text-xs text-slate-500 ml-2">(Optional)</span></label>
            ...
            <div className="bg-white rounded-3xl shadow-[0_32px_64px_-16px_rgba(15,23,42,0.15)] overflow-hidden border border-slate-200">
               ...
               <div className="bg-emerald-50/50 border-t border-slate-100 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div>
                     ...
                     <p className="text-3xl font-extrabold text-slate-950">{getCurrencySymbol()}{convertPrice(priceDetails.payNow).toFixed(2)}</p>
                  </div>
            ...
            <div className="bg-white rounded-card shadow-[0_18px_45px_-34px_rgba(15,23,42,0.5)] border border-slate-200 p-5 sm:p-7">
               <div className="mb-6 flex flex-col gap-4 border-b border-slate-100 pb-5 sm:flex-row sm:items-start sm:justify-between">
                 ...
                 <div className="rounded-card border border-slate-200 bg-slate-50 px-4 py-3">
                   ...
                   <p className="mt-1 text-xl font-extrabold text-slate-950">{getCurrencySymbol()}{convertPrice(priceDetails.payNow).toFixed(2)}</p>
                 </div>
               </div>
               <div className="space-y-6">
                  ...
                  <div className="group">
                    ...
                    {stripeEnabled ? (
                      <div className="rounded-card border border-slate-200 px-4 sm:px-6 py-4 shadow-sm focus-within:ring-4 focus-within:ring-[#007ac2]/10 focus-within:border-[#007ac2] bg-white transition-all">
                        <CardElement options={{ 
          ...
          <div className="lg:col-span-1">
             <div className="sticky top-6 sm:top-10 space-y-5 sm:space-y-6">
                <div className="bg-white rounded-card shadow-[0_22px_55px_-34px_rgba(15,23,42,0.65)] border border-slate-200 p-5 sm:p-6 transition-all duration-500">
                  <div className="mb-5 p-4 rounded-card bg-slate-950 text-white flex items-center justify-between shadow-xl shadow-slate-900/20 relative overflow-hidden group/timer gap-4">
                      <div className="absolute inset-0 bg-[#007ac2] opacity-0 group-hover/timer:opacity-10 transition-opacity"></div>
                      <div>
                          <p className="text-xs font-extrabold text-slate-200 uppercase tracking-[0.25em] mb-1">Session expires</p>
                          <p className="text-sm font-extrabold text-emerald-300 uppercase tracking-[0.15em] flex items-center gap-2"><Clock className="w-3.5 h-3.5"/> Complete now</p>
                      </div>
                      <p className="text-2xl sm:text-3xl font-mono font-extrabold text-white tracking-tighter drop-shadow-lg">{formatTime(timeLeft)}</p>
                  </div>

                   <h3 className="text-base font-extrabold text-slate-950 mb-5 flex items-center gap-3">Reservation details <div className="h-px flex-grow bg-slate-200"></div></h3>
                   ...
                   <button
                     ...
                     className="group relative w-full bg-[#007ac2] hover:bg-[#006607] text-white font-extrabold py-4 rounded-card shadow-2xl shadow-emerald-600/20 transition-all duration-500 active:scale-[0.98] flex items-center justify-center text-xs sm:text-sm uppercase tracking-[0.14em] sm:tracking-[0.22em] disabled:opacity-50 disabled:cursor-not-allowed overflow-hidden"
                   >
                     <div className="absolute inset-0 bg-gradient-to-r from-[#007ac2] to-[#006607] opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
