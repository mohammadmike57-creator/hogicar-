        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50">
            <tr className="text-xs font-semibold">
              <th className="p-3">Company</th><th className="p-3">Contact</th><th className="p-3">Fleet</th><th className="p-3">Integration</th><th className="p-3">Date</th><th className="p-3"></th>
            </tr>
          </thead>
          <tbody>
            {apps.map((app: SupplierApplication) => (
              <tr key={app.id} className="hover:bg-orange-50">
                <td className="p-3"><span className="font-bold">{app.companyName}</span><br/><span className="text-xs text-gray-500">{app.primaryLocation}</span></td>
                <td className="p-3">{app.contactName}<br/><span className="text-xs">{app.email}</span></td>
                <td className="p-3">{app.fleetSize}</td>
                <td className="p-3 uppercase text-xs">{app.integrationType}</td>
                <td className="p-3 text-xs">{app.submissionDate}</td>
                <td className="p-3 text-right">
                  <div className="flex gap-2">
                    <button onClick={() => handleApprove(app)} className="bg-green-100 p-2 rounded-md"><CheckCircle className="w-4 h-4 text-green-700"/></button>
                    <button onClick={() => onReject(app.id)} className="bg-red-100 p-2 rounded-md"><XCircle className="w-4 h-4 text-red-700"/></button>
                  </div>
                </td>
              </tr>
            ))}
