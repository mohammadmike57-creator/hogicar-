...
const InfoTooltip: React.FC<{ text: string }> = ({ text }) => (
  <div className="relative group inline-flex ml-1">
    ...
    <div className="absolute bottom-full left-1/2 -translate-x-1/2 w-64 mb-2 p-3 bg-slate-800 text-white text-xs rounded-card shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
      {text}
...
const RentalConditionsModal = ({ car, supplier, onClose }: { car: Car; supplier: Supplier; onClose: () => void }) => {
  ...
  const PolicyRow = ({ label, value, tone = 'default' }: { label: string; value: React.ReactNode; tone?: 'default' | 'good' | 'warn' }) => (
    <div className="flex items-start justify-between gap-3 border-b border-slate-100 py-2.5 last:border-b-0">
      ...
      <span className={`text-right text-xs font-extrabold ${tone === 'good' ? 'text-[#007ac2]' : tone === 'warn' ? 'text-amber-700' : 'text-slate-900'}`}>{value}</span>
    </div>
  );
  ...
  const ConditionCard = ({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) => (
    <section className="rounded-card border border-slate-200 bg-white p-4 shadow-sm">
      <h4 className="mb-3 flex items-center gap-2 text-sm font-extrabold text-slate-900">
        <span className="flex h-8 w-8 items-center justify-center rounded-card bg-[#007ac2]/10 text-[#007ac2]">{icon}</span>
        {title}
      </h4>
      {children}
    </section>
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/70 z-50 flex items-center justify-center p-3 sm:p-4 backdrop-blur-sm animate-fadeIn" onClick={onClose}>
      <div className="bg-slate-50 rounded-card shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col font-sans overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-start gap-4 p-4 sm:p-5 border-b border-slate-200 bg-white">
          <div className="flex min-w-0 items-center gap-4">
            {(car as any).isHogicarChoiceBranded ? (
              <>
                <div className="w-14 h-14 bg-slate-900 rounded-card flex shrink-0 items-center justify-center shadow-lg border border-amber-500/30">
                  <Award className="w-8 h-8 text-amber-400" />
                </div>
                <div className="min-w-0">
                   <p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-amber-600">Rental conditions</p>
                   <h3 className="text-lg font-extrabold text-slate-950 truncate">Hogicar verified fleet</h3>
                   <p className="text-xs text-amber-600 font-bold uppercase tracking-wider">Hogicar Exclusive Verified Fleet</p>
                </div>
              </>
            ) : (
              <>
                <div className="flex h-14 w-28 shrink-0 items-center justify-center rounded-card border border-slate-200 bg-white px-3 shadow-sm">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-[#007ac2]">Rental conditions</p>
                  <h3 className="text-lg font-extrabold text-slate-950 truncate">{supplier.name}</h3>
        ...
        <div className="p-4 sm:p-5 overflow-y-auto custom-scrollbar">
          <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <div className="rounded-card border border-slate-200 bg-white p-3"><p className="text-[9px] font-extrabold uppercase tracking-widest text-slate-400">Deposit</p><p className="mt-1 text-sm font-extrabold text-slate-950">{depositText}</p></div>
            <div className="rounded-card border border-slate-200 bg-white p-3"><p className="text-[9px] font-extrabold uppercase tracking-widest text-slate-400">Excess</p><p className="mt-1 text-sm font-extrabold text-slate-950">{excessText}</p></div>
            <div className="rounded-card border border-slate-200 bg-white p-3"><p className="text-[9px] font-extrabold uppercase tracking-widest text-slate-400">Mileage</p><p className="mt-1 text-sm font-extrabold text-slate-950">{car.unlimitedMileage ? 'Unlimited' : 'Limited'}</p></div>
            <div className="rounded-card border border-slate-200 bg-white p-3"><p className="text-[9px] font-extrabold uppercase tracking-widest text-slate-400">Pickup</p><p className="mt-1 text-sm font-extrabold text-slate-950">{pickupTypeLabel}</p></div>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[1.45fr_0.9fr]">
            <div className="space-y-4">
              <ConditionCard icon={<CreditCardIcon className="h-4 w-4" />} title="Payment, deposit and card rules">
                ...
                <p className="mt-3 rounded-card bg-amber-50 p-3 text-xs font-semibold leading-relaxed text-amber-800">A physical credit card may be required at the rental desk. Prepaid, virtual, or third-party cards may be refused by the supplier.</p>
              </ConditionCard>

              <ConditionCard icon={<Users className="h-4 w-4" />} title="Required at pick-up">
                <div className="grid gap-2 sm:grid-cols-3">
                  <div className="rounded-card border border-slate-100 bg-slate-50 p-3"><Shield className="mb-2 h-4 w-4 text-[#007ac2]" /><p className="text-xs font-extrabold text-slate-900">Driving license</p><p className="mt-1 text-[11px] font-semibold leading-relaxed text-slate-500">Held for at least 1 year. International permit may be required.</p></div>
                  <div className="rounded-card border border-slate-100 bg-slate-50 p-3"><Users className="mb-2 h-4 w-4 text-[#007ac2]" /><p className="text-xs font-extrabold text-slate-900">Passport or ID</p><p className="mt-1 text-[11px] font-semibold leading-relaxed text-slate-500">A valid photo ID matching the main driver details.</p></div>
                  <div className="rounded-card border border-slate-100 bg-slate-50 p-3"><CreditCardIcon className="mb-2 h-4 w-4 text-[#007ac2]" /><p className="text-xs font-extrabold text-slate-900">Credit card</p><p className="mt-1 text-[11px] font-semibold leading-relaxed text-slate-500">Must have enough available funds for the deposit.</p></div>
                </div>
              </ConditionCard>
            ...
            <aside className="space-y-4">
              <ConditionCard icon={<Building className="h-4 w-4" />} title="Supplier and location">
                ...
                {supplier.address && <div className="mt-3 flex items-start gap-2 rounded-card bg-slate-50 p-3"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-slate-400" /><p className="text-xs font-semibold leading-relaxed text-slate-600">{supplier.address}</p></div>}
              </ConditionCard>
              ...
              <ConditionCard icon={<Clock className="h-4 w-4" />} title="Pickup and return rules">
                ...
                {workingHours.length > 0 && <div className="mt-3 rounded-card border border-slate-100 bg-slate-50 p-3"><p className="mb-2 text-[10px] font-extrabold uppercase tracking-widest text-slate-400">Opening hours</p>{workingHours.map(([day, hours]) => <div key={day} className="flex justify-between gap-3 text-xs"><span className="capitalize font-semibold text-slate-500">{day}</span><span className="text-right font-extrabold text-slate-800">{hours}</span></div>)}</div>}
              </ConditionCard>
            </aside>
          </div>

          <ConditionCard icon={<FileText className="h-4 w-4" />} title="Supplier rental terms">
            <div className="max-h-48 overflow-y-auto rounded-card border border-slate-200 bg-slate-50 p-4 custom-scrollbar">
        ...
        <div className="p-4 bg-white border-t border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button onClick={onClose} className="bg-[#007ac2] text-white px-6 py-3 rounded-card font-extrabold text-sm hover:bg-[#006607] shadow-sm transition-transform active:scale-95">Got it</button>
        </div>
      </div>
    </div>
  );
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center"><Loader2 className="w-12 h-12 animate-spin text-[#007ac2] mx-auto" /><p className="mt-4 text-slate-600">Loading car details...</p></div>
      </div>
    );
  }
  ...
  if (error || !car) {
    return (
      <div className="min-h-screen bg-slate-100/70 flex items-center justify-center p-4">
        <div className="bg-[#f2f5fa] rounded-card p-8 text-center max-w-md shadow-lg border border-slate-300/70">
          ...
          <button onClick={() => navigate(-1)} className="mt-6 bg-[#007ac2] text-white px-6 py-2 rounded-card font-bold hover:bg-[#007ac2]">Back to Search</button>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-slate-50 min-h-screen pb-16 lg:pb-8 text-slate-800">
            ...
            <div className="lg:col-span-2 space-y-6 sm:space-y-8">
                ...
                <div className="grid lg:grid-cols-[0.9fr_1.1fr]">
                  <div className="relative flex min-h-[300px] items-center justify-center bg-gradient-to-br from-slate-50 via-white to-slate-50 p-4 sm:p-8 lg:min-h-[440px] border-b lg:border-b-0 lg:border-r border-slate-100 rounded-t-3xl lg:rounded-l-3xl lg:rounded-tr-none group">
                    ...
                    <div className="absolute top-6 left-6 flex flex-wrap gap-2.5">
                      <span className="hidden md:inline-block bg-slate-950 text-white text-[10px] font-extrabold px-4 py-2 rounded-full uppercase tracking-[0.2em] shadow-lg">
                        {formatCategoryName(car.category)}
                      </span>
                      {car.tags?.[0] && <span className="bg-[#007ac2] text-white text-[10px] font-extrabold px-4 py-2 rounded-full uppercase tracking-[0.2em] shadow-lg shadow-[#007ac2]/20">{car.tags[0]}</span>}
                    </div>
                  ...
                  {car.isHogicarChoiceBranded && (
                    <div className="inline-flex items-center gap-3 bg-slate-950 text-white text-[10px] font-extrabold px-5 py-2.5 rounded-card mb-6 shadow-2xl border border-amber-400/30">
                        <Award className="w-5 h-5 text-amber-400 fill-amber-400/20" />
                        <span className="tracking-[0.25em] uppercase italic font-extrabold text-amber-400">Hogicar Choice Exclusive Verified</span>
                    </div>
                  )}
                  <div className="flex flex-wrap justify-between items-start gap-6">
                    <div className="flex-1 min-w-[240px]">
                      <p className="text-[10px] font-extrabold uppercase tracking-[0.25em] text-[#007ac2] mb-2">Reserved for you</p>
                      <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-950 tracking-tight leading-tight">{car.displayName || `${car.make} ${car.model}`}</h1>
                      ...
                      <div className="flex flex-wrap gap-4 mt-6">
                        <div className="flex items-center gap-2 text-sm font-bold text-slate-900">
                            {!car.isHogicarChoiceBranded ? (
                                <div 
                                  className="flex items-center gap-4 group/rating relative cursor-pointer rounded-card border border-slate-200 bg-white hover:border-[#007ac2]/30 p-3 pr-5 transition-all shadow-sm hover:shadow-xl hover:-translate-y-1 active:scale-95"
                                  onClick={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                ...
                                >
                                    <div className={`relative ${getRatingColor(car.supplier.rating)} text-white w-12 h-12 flex items-center justify-center rounded-card shadow-lg shadow-slate-200 overflow-hidden shrink-0 ring-2 ring-white transition-transform group-hover/rating:scale-110`}>
                                        <div className="absolute inset-0 bg-gradient-to-br from-white/40 to-transparent opacity-50" />
                                        <span className="relative z-10 text-xl font-extrabold">{car.supplier.rating}</span>
                                    </div> 
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-2 mb-0.5">
                                            <span className={`font-extrabold text-lg leading-none ${getRatingTextColor(car.supplier.rating)} tracking-tight`}>
                                        ...
                                        <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.2em] flex items-center gap-1.5">
                                            <CheckCircle className="w-3 h-3 text-[#007ac2]" />
                                            Verified Supplier
                                        </span>
                                    </div>
                                ...
                                <span className="rounded-card border border-amber-200 bg-amber-50 px-4 py-3 font-extrabold text-amber-800 uppercase tracking-wider flex items-center gap-3 shadow-sm">
                    ...
                    <div className="flex flex-col gap-3 items-start sm:items-end">
                      ...
                      {(car.supplier.bookingMode === 'FREE_SALE' || !car.supplier.bookingMode) && (
                        <div className="bg-emerald-50 px-4 py-2.5 rounded-card flex items-center gap-2.5 border border-[#007ac2]/10 shadow-sm">
                          <Zap className="w-4 h-4 text-[#007ac2] fill-[#007ac2]/20" />
                          <span className="text-[10px] font-extrabold text-[#007ac2] uppercase tracking-[0.2em]">Instant confirmation</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-8">
                    {[
                      { icon: Users, label: `${car.passengers}`, desc: 'Seats', color: 'text-[#007ac2]', bg: 'bg-blue-50' },
                      { icon: Briefcase, label: `${car.bags}`, desc: 'Bags', color: 'text-amber-600', bg: 'bg-amber-50' },
                      { icon: car.transmission === 'AUTOMATIC' ? AutomaticIcon : AutomaticIcon, label: car.transmission === 'AUTOMATIC' ? 'Auto' : 'Manual', desc: 'Gear', color: 'text-[#007ac2]', bg: 'bg-emerald-50' },
                      { icon: Snowflake, label: 'A/C', desc: 'Climate', color: 'text-cyan-600', bg: 'bg-cyan-50' },
                    ].map(item => (
                      <div key={item.desc} className="rounded-card border border-slate-200 bg-white p-3 shadow-sm transition-all hover:shadow-md hover:border-slate-300">
                        <div className={`w-8 h-8 ${item.bg} rounded-card flex items-center justify-center mb-2.5 shadow-inner`}>
                  ...
                  <div className="mt-5 grid gap-2 sm:grid-cols-3">
                    <div className="rounded-card bg-emerald-50 border border-emerald-100 p-3">
                      <p className="text-xs font-extrabold text-[#007ac2] flex items-center gap-2"><CheckCircle className="w-4 h-4" /> Free cancellation</p>
                      <p className="text-[11px] text-emerald-800 font-semibold mt-1">Flexible booking before pickup.</p>
                    </div>
                    <div className="rounded-card bg-slate-50 border border-slate-200 p-3">
                      <p className="text-xs font-extrabold text-slate-900 flex items-center gap-2"><GaugeCircle className="w-4 h-4 text-[#007ac2]" /> {car.unlimitedMileage ? 'Unlimited mileage' : 'Limited mileage'}</p>
                      <p className="text-[11px] text-slate-500 font-semibold mt-1">Mileage policy shown before checkout.</p>
                    </div>
                    <div className="rounded-card bg-slate-50 border border-slate-200 p-3">
                      <p className="text-xs font-extrabold text-slate-900 flex items-center gap-2"><Fuel className="w-4 h-4 text-[#007ac2]" /> {car.fuelPolicy.replace(/_/g, ' ')}</p>
              ...
              <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_20px_50px_-20px_rgba(15,23,42,0.15)]">
                <div className="border-b border-slate-100 bg-slate-50/50 px-6 py-5 sm:px-8">
                  ...
                  <div>
                    <h2 className="text-lg font-extrabold flex items-center gap-3 text-slate-950 tracking-tight">
                      <div className="bg-[#007ac2]/10 p-2 rounded-card">
                        <Plane className="w-5 h-5 text-[#007ac2]" />
                      </div>
                  ...
                  <div className="inline-flex items-center gap-2.5 rounded-card border border-[#007ac2]/20 bg-emerald-50/50 px-4 py-2 shadow-sm backdrop-blur-sm">
                    <Clock className="w-4 h-4 text-[#007ac2]" />
                    <span className="text-xs font-extrabold text-[#007ac2] uppercase tracking-[0.1em]">{timeUntilPickup}</span>
                  </div>
                  </div>
                </div>
                ...
                <div className="p-6 sm:p-8 lg:p-10">
                  ...
                  <div className="relative z-10 rounded-card border border-slate-100 bg-white p-6 shadow-sm hover:shadow-md transition-all">
                    <div className="mb-6 flex items-start justify-between gap-4">
                      <span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-card bg-[#007ac2]/5 text-[#007ac2] ring-1 ring-[#007ac2]/10">
                        <PlaneLanding className="w-6 h-6" />
                      </span>
                      <div className="text-right">
                        <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#007ac2]">Pick-up Location</p>
                        <p className="mt-1 text-sm font-extrabold text-slate-900">{pickupCode}</p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div>
                        <p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-slate-400">Date & Time</p>
                        <div className="flex items-center gap-3 mt-1">
                          <p className="text-2xl font-extrabold text-slate-950 tracking-tighter">{startTime}</p>
                          <div className="h-4 w-px bg-slate-200"></div>
                          <p className="text-sm font-extrabold text-slate-900">{pickupDisplay}</p>
                        </div>
                      </div>
                      <div className="pt-4 border-t border-slate-50">
                        <p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-slate-400">Exact address</p>
                  ...
                  <div className="relative z-10 flex flex-col items-center justify-center gap-3">
                    ...
                    <div className="bg-slate-100 px-4 py-1.5 rounded-full border border-slate-200">
                      <span className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-slate-600">{days} days rental</span>
                    </div>
                  </div>

                  <div className="relative z-10 rounded-card border border-slate-100 bg-white p-6 shadow-sm hover:shadow-md transition-all">
                    <div className="mb-6 flex items-start justify-between gap-4">
                      <span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-card bg-[#003580]/5 text-[#003580] ring-1 ring-[#003580]/10">
                        <PlaneTakeoff className="w-6 h-6" />
                      </span>
                      <div className="text-right">
                        <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#003580]">Drop-off Location</p>
                        <p className="mt-1 text-sm font-extrabold text-slate-900">{dropoffCode || pickupCode}</p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div>
                        <p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-slate-400">Date & Time</p>
                        <div className="flex items-center gap-3 mt-1">
                          <p className="text-2xl font-extrabold text-slate-950 tracking-tighter">{endTime}</p>
                          <div className="h-4 w-px bg-slate-200"></div>
                          <p className="text-sm font-extrabold text-slate-900">{dropoffDisplay}</p>
                        </div>
                      </div>
                      <div className="pt-4 border-t border-slate-50">
                        <p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-slate-400">Exact address</p>
              ...
              <div className="bg-white rounded-card shadow-[0_14px_36px_-30px_rgba(15,23,42,0.5)] border border-slate-200 p-5 sm:p-6">
                <div className="flex items-start justify-between gap-4 mb-5">
                  <div>
                    <h2 className="text-xl font-extrabold flex items-center gap-2 text-slate-950"><GaugeCircle className="w-5 h-5 text-[#007ac2]" /> Vehicle details</h2>
                    <p className="text-sm font-semibold text-slate-500 mt-1">The most important specs for this rental class.</p>
                  </div>
                  <span className="hidden sm:inline-flex rounded-full bg-slate-100 px-3 py-1 text-[10px] font-extrabold uppercase tracking-widest text-slate-500">{formatCategoryName(car.category)}</span>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    <div key={spec.label} className="flex flex-col p-3 bg-slate-50 border border-slate-200 rounded-card hover:shadow-sm transition">
                      <spec.icon className="w-5 h-5 text-[#007ac2] mb-2" />
                      <span className="text-sm font-extrabold text-slate-900 leading-tight">{spec.label}</span>
                      <span className="text-[11px] font-semibold text-slate-500">{spec.desc}</span>
                    </div>
                  ))}
                </div>
                <button onClick={() => setShowFullSpecs(!showFullSpecs)} className="mt-5 text-[#007ac2] text-sm font-extrabold flex items-center gap-1 mx-auto">{showFullSpecs ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />} {showFullSpecs ? 'Show less' : 'Show all specs'}</button>
                {showFullSpecs && (
                  <div className="mt-5 p-5 bg-slate-50 border border-slate-200 rounded-card grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              {car.extras && car.extras.length > 0 && (
                <div className="bg-white rounded-card shadow-[0_14px_36px_-30px_rgba(15,23,42,0.5)] border border-slate-200 p-5 sm:p-6">
                  <h2 className="text-xl font-extrabold mb-6 flex items-center gap-2"><PlusCircle className="w-5 h-5 text-[#007ac2]" /> Optional extras</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {car.extras.map(extra => (
                      <div key={extra.id} onClick={() => handleToggleExtra(extra.id)} className={`flex items-center justify-between p-4 rounded-card border-2 cursor-pointer transition-all ${selectedExtraIds.includes(extra.id) ? 'border-[#007ac2] bg-emerald-50' : 'border-slate-200 bg-slate-50 hover:border-slate-300'}`}>
                        <div className="flex items-center gap-3">
                          <div className="p-2 bg-slate-100 rounded-card"><PlusCircle className="w-5 h-5 text-slate-600" /></div>
              ...
              <div className="bg-white rounded-card shadow-[0_14px_36px_-30px_rgba(15,23,42,0.5)] border border-slate-200 p-5 sm:p-6">
                <h2 className="text-xl font-extrabold mb-5 flex items-center gap-2 text-slate-950"><ShieldCheck className="w-5 h-5 text-[#007ac2]" /> Included in your rate</h2>
              ...
              <div className="bg-white rounded-card shadow-[0_14px_36px_-30px_rgba(15,23,42,0.5)] border border-slate-200 p-5 sm:p-6">
                <h2 className="text-xl font-extrabold mb-6 flex items-center gap-2 text-slate-950"><Building className="w-5 h-5 text-[#007ac2]" /> {!car.isHogicarChoiceBranded ? "Supplier and pickup information" : "Hogicar Verification"}</h2>
                {!car.isHogicarChoiceBranded || car.supplier.name === 'Hogi Car Choice' ? (
                  <div className="flex flex-wrap items-center gap-6">
                    ...
                    <div className="flex items-center gap-4">
                        <div>
                            <div className="font-extrabold text-xl text-slate-900 tracking-tight">{car.supplier.name}</div>
                            <div className="text-base text-slate-600 font-medium tracking-wide">Professional Car Rental Provider</div>
                        </div>
                        <div 
                          className="flex items-center gap-4 bg-white p-2.5 pr-4 rounded-card shadow-sm border border-slate-200 ml-2 group/rating relative cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all active:scale-[0.98]"
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            setShowRatingsTooltip(!showRatingsTooltip);
                        ...
                        >
                             <div className={`relative ${getRatingColor(car.supplier.rating)} text-white w-12 h-12 flex items-center justify-center rounded-card shadow-lg shadow-slate-200 overflow-hidden shrink-0 ring-2 ring-white transition-transform group-hover/rating:scale-110 group-hover/rating:rotate-3`}>
                                 <div className="absolute inset-0 bg-gradient-to-br from-white/40 to-transparent opacity-50" />
                                 <span className="relative z-10 text-xl font-extrabold">{car.supplier.rating}</span>
                             </div>
                             <div className="flex flex-col min-w-0">
                                 <div className="flex items-center gap-2 mb-0.5">
                                     <span className={`text-lg font-extrabold leading-none truncate whitespace-nowrap tracking-tight ${getRatingTextColor(car.supplier.rating)}`}>
                                         {getRatingDescription(car.supplier.rating)}
                                     </span>
                                     <Info className="w-3.5 h-3.5 text-slate-300 group-hover/rating:text-slate-500 transition-colors" />
                                 </div>
                                 <span className="text-[10px] font-extrabold text-slate-400 mt-1 uppercase tracking-[0.15em] flex items-center gap-1.5">
                                     <CheckCircle className="w-3 h-3 text-[#007ac2]" />
                                     Verified Rating
                                 </span>
                             </div>
                  ...
                  <div className="flex flex-wrap items-center gap-6">
                    <div className="w-16 h-16 bg-slate-900 rounded-card flex items-center justify-center shadow-xl border border-amber-500/20">
                      <Award className="w-10 h-10 text-amber-400" />
                    </div>
                    <div>
                      <div className="font-extrabold text-xl tracking-tight text-slate-900 uppercase">Hogicar Exclusive Fleet</div>
                      <div className="text-base text-slate-700 font-medium">Verified professional management</div>
                    </div>
                  </div>
                )}
                <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="flex flex-col items-center p-3 bg-slate-50 border border-slate-200 rounded-card"><Shield className="w-6 h-6 text-[#007ac2] mb-1" /><span className="text-sm font-bold text-slate-700">Verified Partner</span></div>
                  <div className="flex flex-col items-center p-3 bg-slate-50 border border-slate-200 rounded-card"><Award className="w-6 h-6 text-yellow-500 mb-1" /><span className="text-sm font-bold text-slate-700">Top Rated</span></div>
                  <div className="flex flex-col items-center p-3 bg-slate-50 border border-slate-200 rounded-card"><Headphones className="w-6 h-6 text-blue-500 mb-1" /><span className="text-sm font-bold text-slate-700">24/7 Support</span></div>
                  <div className="flex flex-col items-center p-3 bg-slate-50 border border-slate-200 rounded-card"><Globe className="w-6 h-6 text-purple-500 mb-1" /><span className="text-sm font-bold text-slate-700">Global Presence</span></div>
                </div>
                <button onClick={() => setIsConditionsModalOpen(true)} className="mt-8 flex items-center gap-2 text-[#007ac2] hover:text-[#006607] text-base font-extrabold uppercase tracking-[0.1em] transition-colors group/cond">
              ...
              {cars && cars.filter(c => c.id !== car.id && c.supplier.id === car.supplier.id && (categoryRanks[c.category] || 0) > (categoryRanks[car.category] || 0)).length > 0 && (
                <div className="bg-white rounded-3xl shadow-[0_20px_50px_-20px_rgba(15,23,42,0.15)] border border-slate-200 p-6 sm:p-8">
                  <div className="flex flex-wrap items-start justify-between gap-4 mb-8">
                    <div>
                      <h2 className="text-xl font-extrabold flex items-center gap-3 text-slate-950 tracking-tight">
                        <div className="bg-amber-100 p-2 rounded-card">
                    ...
                    <div className="flex items-center gap-2 rounded-card border border-slate-100 bg-slate-50 px-4 py-2">
                        <div className="w-2 h-2 rounded-full bg-[#007ac2] animate-pulse"></div>
                        <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500">Available Now</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {cars.filter(c => c.id !== car.id && c.supplier.id === car.supplier.id && (categoryRanks[c.category] || 0) > (categoryRanks[car.category] || 0))
                      ...
                      <button 
                        ...
                        className="group flex h-full flex-col overflow-hidden rounded-card border border-slate-200 bg-white text-left transition-all hover:-translate-y-1 hover:border-[#007ac2]/40 hover:shadow-[0_25px_50px_-12px_rgba(15,23,42,0.25)]"
                      >
                        <div className="relative flex aspect-[16/10] w-full items-center justify-center overflow-hidden bg-slate-50 p-6">
                          ...
                          />
                          <div className="absolute left-4 top-4 rounded-card bg-white/95 px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-widest text-[#007ac2] shadow-sm ring-1 ring-slate-200 backdrop-blur-md">
                            {formatCategoryName(similar.category)}
                          </div>
                        </div>
                        <div className="flex flex-1 flex-col p-5 sm:p-6">
                          <div className="min-w-0">
                            <div className="truncate text-lg font-extrabold text-slate-950 tracking-tight">{similar.displayName}</div>
                            <div className="mt-4 grid grid-cols-3 gap-2.5">
                              <div className="rounded-card border border-slate-100 bg-slate-50 p-2.5 text-center transition-colors group-hover:bg-emerald-50/50 group-hover:border-emerald-100">
                                 <Users className="w-4 h-4 text-[#007ac2] mx-auto mb-1.5" />
                                 <p className="text-xs font-extrabold text-slate-900 leading-none">{similar.passengers}</p>
                              </div>
                              <div className="rounded-card border border-slate-100 bg-slate-50 p-2.5 text-center transition-colors group-hover:bg-emerald-50/50 group-hover:border-emerald-100">
                                 <Briefcase className="w-4 h-4 text-[#007ac2] mx-auto mb-1.5" />
                                 <p className="text-xs font-extrabold text-slate-900 leading-none">{similar.bags}</p>
                              </div>
                              <div className="rounded-card border border-slate-100 bg-slate-50 p-2.5 text-center transition-colors group-hover:bg-emerald-50/50 group-hover:border-emerald-100">
                                 <AutomaticIcon className="w-4 h-4 text-[#007ac2] mx-auto mb-1.5" />
                                 <p className="text-xs font-extrabold text-slate-900 leading-none">{similar.transmission === 'AUTOMATIC' ? 'Auto' : 'Manual'}</p>
                              </div>
                            </div>
                          </div>
                          <div className="mt-6 flex items-center justify-between gap-4 border-t border-slate-100 pt-5">
                            <div>
                              <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-slate-400 mb-1">Total</p>
                              <p className="text-xl font-extrabold text-slate-950">{getCurrencySymbol()}{convertPrice(calcPricing(similar, { pickupDate: startDate, dropoffDate: endDate }).finalTotal).toFixed(2)}</p>
                            </div>
                            <span className="inline-flex h-11 items-center gap-2 rounded-card bg-slate-950 px-5 text-[11px] font-extrabold uppercase tracking-widest text-white transition-all group-hover:bg-[#007ac2] group-hover:shadow-lg group-hover:shadow-[#007ac2]/20">
                              Choose
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-20 space-y-4">
                <div className="bg-white rounded-3xl shadow-[0_32px_64px_-16px_rgba(15,23,42,0.15)] p-6 border border-slate-200 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#007ac2] to-emerald-400"></div>
                  
                  {/* Price lock timer */}
                  <div className="bg-slate-950 text-white p-4 rounded-card mb-6 flex justify-between items-center shadow-lg">
                    <div>
                      <div className="text-[9px] font-extrabold uppercase tracking-[0.2em] text-slate-400">Session Price Lock</div>
                      <div className="font-mono font-extrabold text-2xl tracking-tighter mt-0.5">{formatTime(timeLeft)}</div>
                    </div>
                    <div className="bg-white/10 p-2.5 rounded-card backdrop-blur-md">
                      <Clock className="w-6 h-6 text-emerald-400" />
                    </div>
                  </div>
                  <div className="mb-5 rounded-card border border-slate-200 bg-slate-50 p-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="rounded-card bg-white p-3 ring-1 ring-slate-200">
                        <p className="text-[9px] font-extrabold uppercase tracking-[0.18em] text-slate-400">Pay now</p>
                      ...
                      <div className="rounded-card bg-white p-3 text-right ring-1 ring-slate-200">
                        <p className="text-[9px] font-extrabold uppercase tracking-[0.18em] text-slate-400">At counter</p>
                        <p className="mt-1 text-lg font-extrabold tracking-tight text-slate-950">{getCurrencySymbol()}{convertPrice(priceDetails.payAtDesk).toFixed(2)}</p>
                      </div>
                    </div>
                    <div className="mt-3 flex items-center gap-2 rounded-card bg-emerald-50 px-3 py-2 text-[11px] font-bold text-emerald-800 ring-1 ring-emerald-100">
                      <ShieldCheck className="h-4 w-4 text-[#007ac2]" />
                      Includes mandatory taxes, supplier fees, and booking support.
                    </div>
                  </div>
                  <div className="mb-5">
                    <div className="mb-3 flex items-center justify-between">
                      <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-slate-500">Price breakdown</p>
                      <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[9px] font-extrabold uppercase tracking-wider text-slate-500">{days} days</span>
                    </div>
                    <div className="space-y-3 rounded-card border border-slate-200 bg-white p-4">
                      ...
                      {priceDetails.discountAmount > 0 && <div className="flex justify-between gap-4 rounded-card bg-emerald-50 px-3 py-2 text-sm font-extrabold text-emerald-700"><span>Promo discount</span><span>-{getCurrencySymbol()}{convertPrice(priceDetails.discountAmount).toFixed(2)}</span></div>}
                      {priceDetails.hogicarPromoAmount > 0 && <div className="flex justify-between gap-4 rounded-card bg-indigo-50 px-3 py-2 text-sm font-extrabold text-indigo-700"><span>Secret deal</span><span>-{getCurrencySymbol()}{convertPrice(priceDetails.hogicarPromoAmount).toFixed(2)}</span></div>}
                      <div className="border-t border-dashed border-slate-200 pt-3">
                        <div className="flex items-end justify-between gap-4"><span className="text-sm font-extrabold text-slate-900">Total price</span><span className="text-2xl font-extrabold tracking-tight text-slate-950">{getCurrencySymbol()}{convertPrice(priceDetails.finalTotal).toFixed(2)}</span></div>
                      </div>
                    </div>
                  </div>
                  <div className="mb-5">
                    <div className="flex gap-2 rounded-card border border-slate-200 bg-slate-50 p-2">
                      ...
                      <button onClick={handleApplyPromo} className="rounded-card bg-slate-950 px-4 py-2 text-xs font-extrabold uppercase tracking-wider text-white transition hover:bg-[#007ac2]">Apply</button>
                    </div>
                  ...
                  <div className="mb-5 grid gap-2">
                    <label className={`flex cursor-pointer items-center justify-between rounded-card border p-3 transition ${insuranceOption === 'basic' ? 'border-[#007ac2]/30 bg-emerald-50' : 'border-slate-200 bg-white'}`}>
                      <span className="flex items-center gap-3"><input type="radio" name="insurance" checked={insuranceOption === 'basic'} onChange={() => setInsuranceOption('basic')} className="w-4 h-4 text-[#007ac2]" /><span><span className="block text-sm font-extrabold text-slate-950">Basic cover included</span><span className="text-[11px] font-semibold text-slate-500">Standard supplier coverage</span></span></span>
                      <span className="text-xs font-extrabold text-[#007ac2]">Included</span>
                    </label>
                    <label className={`flex cursor-pointer items-center justify-between rounded-card border p-3 transition ${insuranceOption === 'full' ? 'border-[#007ac2]/30 bg-emerald-50' : 'border-slate-200 bg-white'}`}>
                      <span className="flex items-center gap-3"><input type="radio" name="insurance" checked={insuranceOption === 'full'} onChange={() => setInsuranceOption('full')} className="w-4 h-4 text-[#007ac2]" /><span><span className="block text-sm font-extrabold text-slate-950">Full protection</span><span className="text-[11px] font-semibold text-slate-500">Extra peace of mind</span></span></span>
                      <span className="text-xs font-extrabold text-slate-950">+{getCurrencySymbol()}{convertPrice(15 * days).toFixed(2)}</span>
                    </label>
                  </div>
                  <Link to={`/book/${car.id}/details?${bookingParams}`} onClick={handleContinue} className="btn-primary w-full active:scale-[0.98]">Continue to book</Link>
                  <div className="mt-4 flex items-center justify-center gap-2 opacity-75"><VisaIcon /><MastercardIcon /><AmexIcon /></div>
                </div>
                {/* Trust badge */}
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-[#eaf7ef] rounded-card p-3 border border-green-200/80"><div className="flex gap-2"><ShieldCheck className="w-5 h-5 text-green-600 shrink-0" /><div><div className="text-xs font-extrabold">Free cancellation</div><div className="text-[11px] text-slate-700">Before pickup</div></div></div></div>
                  <div className="bg-white rounded-card p-3 border border-slate-200"><div className="flex gap-2"><Headphones className="w-5 h-5 text-[#007ac2] shrink-0" /><div><div className="text-xs font-extrabold">24/7 support</div><div className="text-[11px] text-slate-700">Anytime help</div></div></div></div>
                </div>
                <div className="bg-white rounded-card p-3 border border-slate-200">
                  <h4 className="text-xs font-extrabold text-slate-900 mb-2">Booking checklist</h4>
        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 p-3 shadow-[0_-18px_40px_-24px_rgba(15,23,42,0.65)] backdrop-blur-xl">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-slate-400">Total price</div>
              <div className="text-xl font-extrabold tracking-tight text-slate-950">{getCurrencySymbol()}{convertPrice(priceDetails.finalTotal).toFixed(2)}</div>
              <div className="text-[10px] font-bold text-[#007ac2]">Pay now {getCurrencySymbol()}{convertPrice(priceDetails.payNow).toFixed(2)}</div>
            </div>
            <Link to={`/book/${car.id}/details?${bookingParams}`} onClick={handleContinue} className="btn-primary px-6 transition active:scale-95">Book now</Link>
          </div>
        </div>
      </div>
    </>
