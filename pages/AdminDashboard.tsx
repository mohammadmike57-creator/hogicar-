...
import { 
  ...
  DestinationItem, FaqItem, CarModel, CarCategory, CarType, Affiliate, 
  SupplierApplication, Car, RateTier, RateByDay, PromoCode 
} from '../types';
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={carModel ? 'Edit Car Model' : 'Add New Car Model'} size="lg">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <SelectField label="Type" value={model.type} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleChange('type', e.target.value)} options={Object.values(CarType).map(v => ({ value: v, label: v }))} />
                </div>
...
const SuppliersContent = ({ suppliers, onEdit, onApprove, onManageApi }: any) => (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
        <div className="flex justify-between items-center mb-4">
            <SectionHeader title="Supplier Management" icon={Building} />
            <button onClick={() => onEdit({})} className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shadow-md">
...
export const AdminDashboard: React.FC = () => {
  ...
  const [managingPromosForCar, setManagingPromosForCar] = useState<Car | null>(null);
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'suppliers':
        return <SuppliersContent suppliers={suppliers} onEdit={setEditingSupplier} onApprove={handleApproveSupplier} onManageApi={(supplier: Supplier) => { setEditingSupplier(supplier); setIsApiModalOpen(true); }} />;
