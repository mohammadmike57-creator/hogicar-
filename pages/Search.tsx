    ...
    <div className="bg-slate-50 min-h-screen pb-12">
      ...
      <div className="bg-slate-950 shadow-lg border-b border-slate-800 md:sticky md:top-[80px] z-30">
        <div className="max-w-[1600px] mx-auto px-2 py-1.5 sm:px-6 sm:py-2.5 lg:px-8">
            <div className="rounded-card border border-slate-700/80 bg-slate-900 shadow-[0_14px_34px_-30px_rgba(0,0,0,0.85)] px-2.5 py-2 sm:px-3 md:py-2">
              <div className="md:hidden">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-sm font-extrabold text-white truncate">{location || 'Select Location'}</p>
                  ...
                  <button
                    ...
                    className="flex h-9 shrink-0 items-center gap-1.5 rounded-card bg-[#007ac2] px-3 text-[10px] font-extrabold uppercase tracking-[0.12em] text-white shadow-md shadow-[#007ac2]/20 active:scale-[0.98]"
                  >
                    <Edit className="h-3 w-3" />
                    <span>{isSearchOpen ? 'Close' : 'Modify'}</span>
                  </button>
                </div>
                <div className="mt-2 flex items-center gap-2 overflow-hidden rounded-card border border-slate-700 bg-slate-800/70 px-2.5 py-1.5">
              ...
              <div className="hidden flex-col gap-1.5 md:flex lg:flex-row lg:items-center lg:justify-between">
                <div className="flex min-w-0 items-center gap-2">
                  <div className="bg-[#007ac2]/15 p-1.5 sm:p-2 rounded-card flex-shrink-0 border border-[#007ac2]/25">
                    <MapPin className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#00a30b]" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm sm:text-base font-extrabold text-white truncate">{location || 'Select Location'}</p>
                ...
                <div className="grid grid-cols-2 gap-1.5 lg:flex lg:min-w-0 lg:flex-1 lg:justify-center">
                  <div className="flex min-w-0 items-center gap-1.5 rounded-card border border-slate-700 bg-slate-800/70 px-2 py-1.5 md:px-2.5 md:py-1.5">
                    <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#00a30b] shrink-0" />
                    <div className="min-w-0">
                      <p className="text-[8px] font-extrabold uppercase tracking-widest text-slate-500">Pickup</p>
                      <p className="text-[10px] sm:text-[11px] font-extrabold text-white truncate">{startDateTimeDisplay}</p>
                    </div>
                  </div>
                  <div className="flex min-w-0 items-center gap-1.5 rounded-card border border-slate-700 bg-slate-800/70 px-2 py-1.5 md:px-2.5 md:py-1.5">
                    <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#00a30b] shrink-0" />
                    <div className="min-w-0">
                      <p className="text-[8px] font-extrabold uppercase tracking-widest text-slate-500">Return</p>
                      <p className="text-[10px] sm:text-[11px] font-extrabold text-white truncate">{endDateTimeDisplay}</p>
                    </div>
                  </div>
                </div>

                <div className="grid grid-cols-[1fr_auto] sm:grid-cols-[1fr_1fr_auto] items-center gap-1.5">
                  <div className="hidden sm:block rounded-card bg-slate-800/70 border border-slate-700 px-2.5 py-1.5">
                    <p className="text-[8px] font-extrabold text-slate-500 uppercase tracking-widest">Days</p>
                    <p className="text-[11px] font-extrabold text-white">{days}</p>
                  </div>
                  <div className="hidden sm:block rounded-card bg-slate-800/70 border border-slate-700 px-2.5 py-1.5">
                    <p className="text-[8px] font-extrabold text-slate-500 uppercase tracking-widest">Suppliers</p>
                    <p className="text-[11px] font-extrabold text-white">{allSuppliers.length}</p>
                  </div>
                  <div className="sm:hidden rounded-card bg-slate-800/70 border border-slate-700 px-2.5 py-1.5">
                    <p className="text-[8px] font-extrabold text-slate-500 uppercase tracking-widest">Trip</p>
                    <p className="text-[10px] font-extrabold text-white">{days} day{days > 1 ? 's' : ''}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setIsSearchOpen(!isSearchOpen)}
                    className="h-full flex items-center justify-center gap-1.5 text-white font-extrabold text-[10px] sm:text-xs px-3 sm:px-4 rounded-card bg-[#007ac2] hover:bg-[#006607] transition-all shadow-md shadow-[#007ac2]/20 active:scale-[0.98] whitespace-nowrap"
                  >
      ...
      <div className="hidden md:block bg-white border-b border-slate-200 py-4 md:py-5">
          <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 gap-3">
                <div>
                  <h2 className="text-lg sm:text-2xl font-extrabold text-slate-950 tracking-tight flex items-center gap-2">
                      <CarIcon className="w-5 h-5 text-[#007ac2]"/>
                      Car Categories
                  </h2>
                  <p className="text-xs font-semibold text-slate-500 mt-1">Prices show the lowest total for {days} day{days > 1 ? 's' : ''}.</p>
                </div>
                <div className="hidden sm:flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-card px-3 py-1.5">
                    <span className="text-xs text-slate-500 font-bold uppercase tracking-wider">Sort by:</span>
                    <div className="relative">
                        <select 
                            value={sortBy}
                            onChange={(e) => setSortBy(e.target.value)}
                            className="appearance-none bg-transparent text-sm text-slate-900 font-bold rounded-card pr-8 py-0.5 focus:outline-none cursor-pointer"
                        >
                            <option>Recommended</option>
                            <option>Price: Low to High</option>
                            <option>Price: High to Low</option>
                        </select>
                        <ArrowUpDown className="w-3.5 h-3.5 text-[#007ac2] absolute right-0 top-1/2 -translate-y-1/2 pointer-events-none" />
                    </div>
                </div>
              </div>
              <div className="relative">
                    ...
                    {categoryOrder.map(category => {
                        ...
                        return (
                            <button
                                ...
                                disabled={!hasCars}
                                className={`flex-shrink-0 w-[118px] sm:w-[150px] md:w-[162px] flex flex-col rounded-card transition-all duration-300 relative border group snap-start overflow-hidden
                                    ${isActive
                                        ? 'bg-white border-[#007ac2] shadow-[0_16px_34px_-18px_rgba(0,128,9,0.65)] ring-2 ring-[#007ac2]/10'
                                        : hasCars
                                          ? 'bg-white border-slate-200 hover:border-[#007ac2]/50 hover:shadow-[0_16px_32px_-22px_rgba(15,23,42,0.7)]'
                                          : 'bg-slate-50 border-slate-200 opacity-55 cursor-not-allowed'}`}
                            >
                                <div className="px-2.5 pt-2.5 sm:px-3 sm:pt-3 text-center">
                                    <span className={`block truncate text-xs sm:text-sm font-extrabold tracking-tight ${isActive ? 'text-[#007ac2]' : 'text-slate-900'}`}>
                                ...
                                <div className="mt-auto border-t border-slate-100 px-2.5 py-2 sm:px-3 sm:py-2.5 text-center">
                                    <span className="block text-[9px] sm:text-[10px] font-extrabold uppercase tracking-[0.12em] text-slate-400">
                                        {hasCars ? `${count} available` : 'No cars'}
                                    </span>
                                    <span className={`mt-0.5 block text-[11px] sm:text-[12px] font-semibold ${isActive ? 'text-[#007ac2]' : 'text-slate-700'}`}>
                                        {summary?.fromTotal != null ? (
                                          <>from <strong className="text-sm sm:text-base font-extrabold text-slate-950">{getCurrencySymbol()}{convertPrice(summary.fromTotal).toFixed(0)}</strong></>
                                        ) : (
                                          'Unavailable'
                                        )}
                                    </span>
                                ...
                                {isActive && (
                                  <div className="absolute right-2 top-2 bg-[#007ac2] text-white rounded-full p-1 shadow-lg z-10 animate-in zoom-in-50 duration-300">
      ...
      <div className="max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8 pt-0 md:pt-5">
        ...
        <div className="md:hidden my-2 bg-white/95 backdrop-blur p-1.5 border border-slate-200 rounded-card sticky top-2 z-20 grid grid-cols-2 gap-1.5 shadow-[0_14px_36px_-28px_rgba(15,23,42,0.7)]">
            <button 
                onClick={() => {
                    setShowMobileSort(false);
                    setShowMobileFilters(true);
                }}
                className="flex items-center justify-center gap-2 bg-slate-50 border border-slate-200 text-slate-800 px-3 py-2.5 rounded-card font-extrabold text-[11px] shadow-sm active:scale-[0.98] transition-all hover:bg-slate-50"
            >
                <SlidersHorizontal className="w-4 h-4 text-[#007ac2]" />
                <span>Filters</span>
                { activeFilterCount > 0 && (
                    <span className="bg-[#007ac2] text-white w-4 h-4 rounded-full flex items-center justify-center text-[8px]">
                        {activeFilterCount}
                    </span>
                )}
            </button>
            <button 
                ...
                className="flex items-center justify-center gap-2 bg-slate-50 border border-slate-200 text-slate-800 px-3 py-2.5 rounded-card font-extrabold text-[11px] shadow-sm active:scale-[0.98] transition-all hover:bg-slate-50 min-w-0"
            >
                {sortBy === 'Recommended' ? (
                    <Gem className="w-4 h-4 text-[#007ac2]" />
                ) : (
                    <ArrowUpDown className="w-4 h-4 text-[#007ac2]" />
                )}
        ...
        <div className="flex flex-col md:flex-row gap-5 lg:gap-7 items-start">
          ...
          `}>
            ...
            <aside className={`
              ...
              md:h-auto md:mt-0 md:rounded-card md:shadow-[0_18px_45px_-34px_rgba(15,23,42,0.55)] md:border md:border-slate-200
            ...
            `}>
              <div className="flex items-center justify-between p-5 md:p-4 border-b border-slate-100 md:bg-slate-950 md:text-white md:rounded-t-card">
                <div className="flex items-center gap-2">
                    <h3 className="font-extrabold text-slate-800 md:text-white flex items-center gap-2 text-sm uppercase tracking-wider">
                      <SlidersHorizontal className="w-4 h-4 text-[#007ac2]" /> Filters
                    </h3>
                    {showMobileFilters && (
                        <span className="md:hidden bg-emerald-50 text-[#007ac2] text-[10px] px-2 py-0.5 rounded-full font-bold">
                ...
                <div className="flex items-center gap-4">
                    <button 
                      ...
                      className="text-[10px] text-[#007ac2] md:text-emerald-300 font-extrabold hover:underline uppercase tracking-widest"
                    >
                      Reset
                    </button>
                    <button 
              ...
              <div className="divide-y divide-slate-100 overflow-y-auto md:overflow-visible flex-1 md:flex-none custom-scrollbar pb-24 md:pb-0">
                  <div className="p-4 md:p-3 bg-emerald-50/60 md:bg-white">
                      <label className="flex items-center cursor-pointer hover:bg-white md:hover:bg-slate-50 p-2 rounded-card -ml-1 border border-emerald-100 md:border-transparent">
                          <input type="checkbox" checked={specialOffersOnly} onChange={(e) => setSpecialOffersOnly(e.target.checked)} className="rounded w-4 h-4 text-[#007ac2] shadow-sm focus:border-[#007ac2] focus:ring focus:ring-[#007ac2] focus:ring-opacity-50" />
                  ...
                  <div className="p-4 md:p-3">
                      <button onClick={() => toggleFilterSection('Price')} className="w-full flex justify-between items-center text-left group">
                          <span className="text-xs font-extrabold text-slate-800 group-hover:text-[#007ac2] uppercase tracking-wide">Price per Day</span>
                      ...
                      {openFilters.includes('Price') && (
                          <div className="mt-3 px-1">
                              ...
                              <input 
                                  ...
                                  className="w-full h-1.5 bg-slate-200 rounded-card appearance-none cursor-pointer accent-[#007ac2]"
                              />
                          </div>
                      )}
                  </div>
                  ...
                  <div className="hidden md:block p-4 md:p-3">
                      <button onClick={() => toggleFilterSection('Category')} className="w-full flex justify-between items-center text-left group">
                          <span className="text-xs font-extrabold text-slate-800 group-hover:text-[#007ac2] uppercase tracking-wide">Car Category</span>
                              ...
                              {allCategories.map((type) => {
                                  ...
                                  return (
                                      <button 
                                          ...
                                          className={`flex flex-col items-center justify-center p-2.5 rounded-card border transition-all duration-300 ${
                                              isActive 
                                                  ? 'bg-[#007ac2] border-[#007ac2] shadow-lg shadow-emerald-100' 
                                                  : 'bg-white border-slate-200 hover:border-[#007ac2]/40 hover:bg-slate-50'
                                          }`}
                                      >
                                          <span className={`text-[10px] font-extrabold text-center leading-tight transition-colors ${isActive ? 'text-white' : 'text-slate-700'}`}>
                  ...
                  <div className="p-4 md:p-3">
                      <button onClick={() => toggleFilterSection('Passengers')} className="w-full flex justify-between items-center text-left group">
                          <span className="text-xs font-extrabold text-slate-800 group-hover:text-[#007ac2] uppercase tracking-wide">Number of seats</span>
                      ...
                      {openFilters.includes('Passengers') && (
                          <div className="mt-2 grid grid-cols-4 gap-2">
                              {[2, 4, 5, 7].map(num => (
                                  <button key={num} onClick={() => setPassengerCapacity(passengerCapacity === num ? 0 : num)} className={`p-2 border rounded-md text-xs font-bold transition-colors ${passengerCapacity === num ? 'bg-[#007ac2] text-white border-[#007ac2]' : 'bg-white text-slate-700 border-slate-200 hover:border-[#007ac2]'}`}>
                                      {num}{num === 7 ? '+' : ''}
                                  </button>
                              ))}
                          </div>
                  ...
                  <div className="p-4 md:p-3">
                      <button onClick={() => toggleFilterSection('Payment')} className="w-full flex justify-between items-center text-left group">
                          <span className="text-xs font-extrabold text-slate-800 group-hover:text-[#007ac2] uppercase tracking-wide">Payment Type</span>
                          ...
                          <div className="mt-2 space-y-1.5">
                              {allPaymentTypes.map((type) => (
                                  <label key={type} className="flex items-center cursor-pointer hover:bg-slate-50 p-1 rounded -ml-1">
                                      <input type="checkbox" checked={selectedPaymentTypes.includes(type)} onChange={() => handlePaymentTypeChange(type)} className="rounded w-4 h-4 text-[#007ac2]" />
                  ...
                  <div className="p-4 md:p-3">
                      <button onClick={() => toggleFilterSection('Deposit')} className="w-full flex justify-between items-center text-left group">
                          <span className="text-xs font-extrabold text-slate-800 group-hover:text-[#007ac2] uppercase tracking-wide">Refundable Security Deposit</span>
                          ...
                          <div className="mt-2 space-y-1.5">
                              {[0, 300, 500].map((amount) => (
                                  <label key={amount} className="flex items-center cursor-pointer hover:bg-slate-50 p-1 rounded -ml-1">
                                      <input type="radio" name="deposit" checked={maxDeposit === amount} onChange={() => setMaxDeposit(amount)} className="w-4 h-4 text-[#007ac2] focus:ring-[#007ac2]" />
                  ...
                  <div className="p-4 md:p-3">
                      <button onClick={() => toggleFilterSection('LocationType')} className="w-full flex justify-between items-center text-left group">
                          <span className="text-xs font-extrabold text-slate-800 group-hover:text-[#007ac2] uppercase tracking-wide">Location Type</span>
                      ...
                      {openFilters.includes('LocationType') && (
                          <div className="mt-2 space-y-1.5">
                              {allLocationTypes.map((type) => (
                                  <label key={type} className="flex items-center cursor-pointer hover:bg-slate-50 p-1 rounded -ml-1">
                                      <input type="checkbox" checked={selectedLocationTypes.includes(type)} onChange={() => handleLocationTypeChange(type)} className="rounded w-4 h-4 text-[#007ac2]" />
                  ...
                  <div className="p-4 md:p-3">
                      <button onClick={() => toggleFilterSection('Transmission')} className="w-full flex justify-between items-center text-left group">
                          <span className="text-xs font-extrabold text-slate-800 group-hover:text-[#007ac2] uppercase tracking-wide">Transmission</span>
                      ...
                      {openFilters.includes('Transmission') && (
                          <div className="mt-2 space-y-1.5">
                              {allTransmissions.map((type) => (
                                  <label key={type} className="flex items-center cursor-pointer hover:bg-slate-50 p-1 rounded -ml-1">
                                      <input type="checkbox" checked={selectedTransmissions.includes(type)} onChange={() => handleTransmissionChange(type)} className="rounded w-4 h-4 text-[#007ac2]" />
                                      <span className="ml-2 text-xs text-slate-600 font-medium">{type}</span>
                  ...
                  <div className="p-4 md:p-3">
                      <button onClick={() => toggleFilterSection('Fuel')} className="w-full flex justify-between items-center text-left group">
                          <span className="text-xs font-extrabold text-slate-800 group-hover:text-[#007ac2] uppercase tracking-wide">Fuel Policy</span>
                          ...
                          <div className="mt-2 space-y-1.5">
                              {allFuelPolicies.map((policy) => (
                                  <label key={policy} className="flex items-center cursor-pointer hover:bg-slate-50 p-1 rounded -ml-1">
                                      <input type="checkbox" checked={selectedFuelPolicies.includes(policy)} onChange={() => handleFuelPolicyChange(policy)} className="rounded w-4 h-4 text-[#007ac2]" />
                  ...
                  <div className="p-4 md:p-3">
                      <button onClick={() => toggleFilterSection('Supplier')} className="w-full flex justify-between items-center text-left group">
                          <span className="text-xs font-extrabold text-slate-800 group-hover:text-[#007ac2] uppercase tracking-wide">Suppliers in {pickupIata || location}</span>
                          ...
                          <div className="mt-2 space-y-1.5">
                              ...
                              {allSuppliers.map((name) => (
                                  <label key={name} className="flex items-center cursor-pointer hover:bg-slate-50 p-1 rounded -ml-1">
                                      <input type="checkbox" checked={selectedSuppliers.includes(name)} onChange={() => handleSupplierChange(name)} className="rounded w-4 h-4 text-[#007ac2]" />
              ...
              <div className="p-4 border-t border-slate-100 md:hidden bg-white shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.1)] absolute bottom-0 left-0 right-0 z-10">
                  <button 
                    onClick={() => setShowMobileFilters(false)}
                    className="w-full bg-[#007ac2] text-white py-4 rounded-card font-bold text-sm shadow-xl shadow-[#007ac2]/30 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                  >
            ...
            `}>
              <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 flex items-center gap-2 text-sm uppercase tracking-wider">
                  <ArrowUpDown className="w-4 h-4 text-[#007ac2]" /> Sort By
                </h3>
                <button 
                  onClick={() => setShowMobileSort(false)}
              ...
              <div className="p-4 space-y-3 pb-12">
                {[
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-5 rounded-card transition-all ${
                      sortBy === option.label 
                        ? 'bg-[#007ac2] border-2 border-[#007ac2] text-white shadow-lg shadow-emerald-200' 
                        : 'bg-slate-50 border-2 border-transparent text-slate-600 hover:bg-slate-100'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-card ${sortBy === option.label ? 'bg-white/20' : 'bg-white shadow-sm'}`}>
                            {React.cloneElement(option.icon as React.ReactElement<any>, { 
                                className: `w-4 h-4 ${sortBy === option.label ? 'text-white' : 'text-[#007ac2]'}` 
                            })}
                        </div>
                        <span className="font-bold text-sm">{option.label}</span>
                    </div>
                    {sortBy === option.label && <div className="bg-white rounded-full p-1"><Check className="w-3 h-3 text-[#007ac2]" /></div>}
                  </button>
                ))}
              </div>
            </aside>
          ...
          <main className="flex-grow w-full">
            {loading ? (
              ...
              <div className="text-center bg-white rounded-card shadow-sm border border-red-200 py-12 px-6">
                ...
                <>
                <div className="flex items-center justify-between gap-3 mb-4 bg-white border border-slate-200 rounded-card shadow-sm p-4 md:p-4">
                    <div className="min-w-0">
                      <p className="text-base md:text-sm text-slate-900 font-extrabold uppercase tracking-tight md:tracking-wide">
                          <span className="text-[#007ac2]">{sortedAndFilteredCars.length}</span> cars available
                      </p>
                      <p className="text-[11px] text-slate-500 font-bold mt-1">
                        {pickupIata || location || 'Selected location'} • {days} day{days > 1 ? 's' : ''} • {sortBy}
                      </p>
                    </div>
                    <div className="flex max-[420px]:hidden items-center gap-2 text-[10px] text-slate-500 font-extrabold uppercase tracking-widest text-right rounded-full bg-emerald-50 px-3 py-1.5">
                        <Check className="w-3 h-3 text-[#007ac2]" /> Taxes & fees included
                    </div>
                </div>
                <div className="grid grid-cols-1 gap-4 md:gap-3 px-0">
                    ...
                    {sortedAndFilteredCars.length === 0 && (
                         <div className="col-span-full text-center bg-white rounded-card shadow-sm border border-slate-200 py-12 px-6">
      ...
      {selectedCompareCars.length === 0 && sortedAndFilteredCars.length > 0 && (
        ...
        <div className="fixed bottom-4 left-1/2 z-[100] w-[94%] max-w-5xl -translate-x-1/2 animate-in slide-in-from-bottom-8 duration-700 md:hidden">
            <button
                ...
                className="flex w-full items-center justify-between gap-4 rounded-card border border-slate-200 bg-white/95 p-4 text-left text-slate-950 shadow-[0_22px_58px_-30px_rgba(15,23,42,0.55)] ring-1 ring-slate-900/5 backdrop-blur-2xl sm:rounded-3xl sm:px-6"
                aria-pressed={isCompareMode}
            >
                <div className="flex min-w-0 items-center gap-3">
                    <Sparkles className="h-6 w-6 shrink-0 text-slate-950" />
                    <div className="min-w-0">
                        <p className="text-lg font-extrabold leading-tight text-slate-950 sm:text-xl">Smart comparison</p>
                        <p className="mt-1 text-sm font-medium text-slate-600 sm:text-base">Compare prices, specs, ratings and more</p>
                    </div>
                </div>
                <span className={`relative flex h-8 w-14 shrink-0 items-center rounded-full p-1 transition-colors ${isCompareMode ? 'bg-[#007ac2]' : 'bg-slate-300'}`}>
      ...
      {selectedCompareCars.length > 0 && (
        <div className="fixed bottom-4 left-1/2 z-[100] w-[94%] max-w-5xl -translate-x-1/2 animate-in slide-in-from-bottom-8 duration-700 sm:bottom-6">
            <div className="relative overflow-hidden rounded-card border border-slate-200 bg-white/95 p-3 text-slate-950 shadow-[0_26px_70px_-28px_rgba(15,23,42,0.65)] ring-1 ring-slate-900/5 backdrop-blur-2xl sm:rounded-3xl sm:p-4">
                <div 
                    className="absolute bottom-0 left-0 h-1 bg-[#007ac2] transition-all duration-500 ease-out"
                    style={{ width: `${(selectedCompareCars.length / 4) * 100}%` }}
                />
              
                <div className="relative z-10 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="flex min-w-0 items-center gap-3 sm:gap-4">
                        <div className="hidden h-12 w-12 shrink-0 items-center justify-center rounded-card bg-[#007ac2] text-white shadow-[0_16px_32px_-18px_rgba(0,128,9,0.9)] sm:flex">
                            <ArrowLeftRight className="h-5 w-5 stroke-[2.5px]" />
                        </div>
                        <div className="min-w-0">
                            <p className="text-sm font-extrabold uppercase tracking-[0.14em] text-slate-950 md:text-base">Compare your choices</p>
                            ...
                            <div className="mt-2 flex items-center gap-2 sm:mt-2.5">
                                <div className="flex gap-1.5">
                                    {[1, 2, 3, 4].map(i => (
                                        <div key={i} className={`h-1.5 w-5 rounded-full transition-colors ${i <= selectedCompareCars.length ? 'bg-[#007ac2]' : 'bg-slate-200'}`} />
                                    ))}
                                </div>
                                <span className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-slate-500">{selectedCompareCars.length}/4 choices</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-3 md:shrink-0 md:justify-end md:gap-6">
                        <div className="flex min-w-0 -space-x-2 sm:-space-x-3">
                            {selectedCompareCars.map(car => (
                                <div key={car.id} className="relative group/comp">
                                    <div className="flex h-12 w-12 cursor-pointer items-center justify-center overflow-hidden rounded-card border-2 border-white bg-slate-50 p-1.5 shadow-lg ring-1 ring-slate-200 transition-all duration-300 hover:-translate-y-1 hover:scale-105 hover:ring-[#007ac2]/40 sm:h-14 sm:w-14 md:h-16 md:w-16">
                            ...
                            {Array.from({ length: 4 - selectedCompareCars.length }).map((_, i) => (
                                <div key={i} className="flex h-12 w-12 items-center justify-center rounded-card border-2 border-dashed border-slate-200 bg-slate-50 sm:h-14 sm:w-14 md:h-16 md:w-16">
                        ...
                        <button 
                            ...
                            className={`
                                relative group/btn min-h-12 shrink-0 overflow-hidden rounded-card px-5 py-3 text-xs font-extrabold uppercase tracking-[0.16em] transition-all duration-300 sm:rounded-card sm:px-7 md:px-9
                                ${selectedCompareCars.length >= 2 
                                    ? 'bg-[#007ac2] text-white shadow-[0_18px_38px_-18px_rgba(0,128,9,0.8)] hover:bg-slate-950 active:scale-95' 
