...
const StatCard = ({ icon: Icon, title, value, change, color = "orange" }: any) => {
  ...
  return (
    <motion.div 
      whileHover={{ y: -5 }}
      className="bg-white p-6 rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 flex flex-col justify-between"
    >
      <div className="flex justify-between items-start mb-4">
        <div className={`p-3 rounded-2xl ${colors[color].split(' ').slice(-2).join(' ')}`}>
          <Icon className="w-6 h-6" />
        </div>
        {change && (
          <span className={`text-[10px] font-black px-2 py-1 rounded-lg uppercase tracking-wider ${change.startsWith('+') ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
            {change}
          </span>
        )}
      </div>
...
const SupplierDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfd] flex font-sans text-gray-900">
      {/* Sidebar */}
      <motion.aside 
        initial={false}
        animate={{ width: isSidebarOpen ? 280 : 0, opacity: isSidebarOpen ? 1 : 0 }}
        className="fixed inset-y-0 left-0 bg-white border-r border-gray-100 z-50 flex flex-col overflow-hidden shadow-2xl shadow-gray-200/20"
      >
        <div className="p-8 mb-4">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="font-black text-xl tracking-tighter text-gray-900">HOGICAR</h1>
                    <div className="flex items-center gap-1.5">
                        <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Supplier Portal</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto custom-scrollbar">
            {[
                ...
                >
                    <item.icon className={`w-5 h-5 ${activeSection === item.id ? 'text-white' : 'group-hover:text-orange-600 transition-colors'}`} />
      ...
      <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'ml-[280px]' : 'ml-0'}`}>
        {/* Top Header */}
        <header className="sticky top-0 bg-white/80 backdrop-blur-md z-40 border-b border-gray-100 px-8 py-5 flex justify-between items-center">
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                    <img src={supplier.logoUrl || 'https://placehold.co/40x40/orange/white?text=S'} className="w-10 h-10 rounded-xl object-cover border border-gray-100 shadow-sm" alt={supplier.name} />
                    <div>
                        <h2 className="text-sm font-black text-gray-900 tracking-tight">{supplier.name}</h2>
                        <div className="flex items-center gap-1.5">
                            <MapPin className="w-3 h-3 text-gray-400" />
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{supplier.locationCode || 'Global'}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4">
                <div className="hidden md:flex flex-col text-right mr-4">
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Account Status</span>
                    <Badge variant={supplier.active ? 'success' : 'error'}>{supplier.active ? 'Verified Partner' : 'Suspended'}</Badge>
                </div>
                <button className="relative p-2.5 hover:bg-gray-100 rounded-2xl transition-colors">
